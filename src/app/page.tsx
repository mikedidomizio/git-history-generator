...
const Commit = (props: CommitProps) => {
  return <div className="bg-[#F05033] text-white rounded-full p-3">
    ...
    {props.arrowTo?.map((arrow) => {
      return <Arrow color={props?.dimmed ? '#F05033' : '#F05033'} key={arrow.to} arrow={arrow.to} dashedArrow={arrow.dashed || false} id={props.id} />
    })}
  </div>
}
...
const Branch = (props: BranchProps) => {
  return <div className="bg-blue-600 text-white p-3">
    {props.text}
    {props.arrowTo?.map((arrow) => {
      return <Arrow color={'#F05033'} key={arrow.to} arrow={arrow.to} dashedArrow={arrow.dashed || false} id={props.id} />
    })}
  </div>
}
